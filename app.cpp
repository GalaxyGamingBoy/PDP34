...
void processData() {
    ...
    votingData[2] = std::stoi(parameters[1][1]);
    votingData[1] = std::stoi(parameters[1][2]);
...
int main() {
    readFile("voting.in");
    processData();
    if (votingData[1] <= votingData[0] && votingData[0] <= 1000000 && votingData[1] >= 1 && votingData[0] >= 1 && votingData[2] >= 1 && votingData[2] <= 1000) {
        addCandidates();
        countVotes();
        declareCandidate();
